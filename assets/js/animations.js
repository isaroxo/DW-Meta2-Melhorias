...
let pesquisaMobile_ativa = false;
let lupaMobileExterna = document.querySelector("#PesquisaMobile>div:first-of-type");
let barraPesquisaMobile = document.querySelector(".BarraPesquisaMobile");
...
document.addEventListener("keydown", function (evt) {
    ...
    if (deviceWidth >768) {
        ...
        if (key === "Enter") {
            recarregarProjetos(alfabetica, temporal_crescente);
            let inputPesquisaMobile = document.querySelector(".BarraPesquisaMobile input");
            let lupaMobileInterna = document.querySelector(".BarraPesquisaMobile i");

            lupaMobileInterna.style.left = "70%";
            setTimeout(function () {
                barraPesquisaMobile.classList.remove("mudarOpacidade");
                setTimeout(function () {
                    barraPesquisaMobile.classList.remove("mostrarBarraPesquisa");
                }, 500);
                body.style.overflowY = "scroll";
                lupaMobileInterna.style.left = "0";
                pesquisaMobile_ativa = false;
                let inputValue = inputPesquisaMobile.value;
                if (inputValue !== "") {
...
lupaMobileExterna.addEventListener("click", function () {
    pesquisaMobile_ativa = true;
    body.style.overflowY = "hidden";
    barraPesquisaMobile.classList.add("mostrarBarraPesquisa");
    setTimeout(function () {
        barraPesquisaMobile.classList.add("mudarOpacidade");
    }, 100);
});
...
function pesquisar(texto) {
    ...
    for (let j = 0; todosProjetos.length; j++) {
        if ((projetos[j].titulo).toLowerCase().includes(textoSemUnderscores) === false || projetos[j].titulo[0].toLowerCase() !== textoSemUnderscores[0]) {
            ...
            todosProjetos[j].style.opacity = "100%";
            linksToRemove[j].setAttribute("href", projetos[j].nomePesquisa + ".php");
            linksToRemove[j].style.cursor = "pointer";
        }
    }
}
